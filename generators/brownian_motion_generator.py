...
class BrownianMotionDataset(Dataset):
    ...
    def __init__(self, batch_size: int = 16, num_of_positives=4, series_length: int = 1000, subseries_length: int = 50, stride: int = 50, intensity_count=100, ranges= None):
        """
        Args:
        ...
        # Randomly select 'parameter_count' different (mean, std) pairs
        if ranges is None:
            ranges = {
                "means": (0, 0),
                "std": (0.5, 10)
        ...
        if ranges["means"] == (0, 0):
            self.means = np.zeros(intensity_count)
        else:
            self.means = np.random.uniform(*ranges["means"], size=intensity_count)
        self.stds = np.random.uniform(*ranges["std"], size=intensity_count)
    ...
    def get_parameters(self, labels):
        res = []
        for label in labels:
            res.append([self.means[label], self.stds[label]])
        return torch.tensor(res)

    def _generate_brownian_series(self, label):
        """
        ...
        full_series = np.random.normal(self.means[label], self.stds[label], self.series_length)
    ...
    def __len__(self):
        ...
        """
        return self.num_of_positives * len(self.means)
    ...
    def __getitem__(self, index: int):
        ...
        """
        label = index % (len(self.means) * self.num_of_positives) // self.num_of_positives
        result = self._generate_brownian_series(label)
        return result, label
...
if __name__ == "__main__":
    # Create the dataset
    dataset = BrownianMotionDataset(batch_size=16, series_length=1000, subseries_length=50, stride=50)
