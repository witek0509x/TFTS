...
from generators.backbone_esn import EchoStateNetwork
...
class EchoStateDataset(Dataset):
    def __init__(self, config: Optional[Dict[str, Any]] = None,
                 ...
                 reset_every=-1, esn_store_state=False, chamber_label=False, input_label=False):
        """
        ...
        print("running backbone dataset")
        ...
        self.chamber_label = chamber_label
    ...
    def __getitem__(self, idx):
        ...
        series, series_n = self.esn.generate_series(self.series_length, roll_every=self.roll_every)

        if self.chamber_label:
            label = self.esn.get_hidden_states()
        elif self.input_label:
            label = torch.tensor(self.esn.input)
        else:
            label = torch.tensor(idx)
        return torch.Tensor(np.array([series, series_n]).T), label
