...
class TrivialNN(LightningModule):

    def __init__(self, config: Optional[Dict[str, Any]] = None, loss_fn=None, d_model=256, nhead=16,
                 num_layers=8, dim_feedforward=1024, input_dim=1, input_len=100, lr=1e-4, masking_ratio=0.2):
        """
        ...
        if config is not None:
            ...
            input_len = model_config.get('input_len', input_len)
        ...
        self.embedding = nn.Linear(input_dim * input_len, d_model)

        self.up_scale = nn.Linear(d_model, dim_feedforward)
        self.softmax = nn.Softmax(dim=-1)
        self.down_scale = nn.Linear(dim_feedforward, d_model)

        self.linear = nn.Linear(d_model, input_len)
...
if __name__ == "__main__":
    ...
    config = load_config("/home/wojciech/private/magisterka/TFTS/configs/transformer_mlm/trivial_nn_config.yaml")
